...
import gpflow
from gpflow.params import Parameter, Parameterized#, AutoFlow
...
class KernelList(gpflow.params.Parameterized):
...
class BlockLookupKernel(KernelKeeper, BlockKernel):
    ...
    def subKdiag(self, index, X, Y = None):
        i = index
        if Y is None:
            Y = X
        return self.kerns[i,i].Kdiag(X, presliced = True)
